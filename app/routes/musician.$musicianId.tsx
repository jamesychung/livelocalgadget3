...
export default function MusicianProfile() {
  ...
  const [musicianData, setMusicianData] = useState<MusicianData | null>(null);
  const [musicianFetching, setMusicianFetching] = useState(true);
  const [musicianError, setMusicianError] = useState<Error | null>(null);
  const [bookingsData, setBookingsData] = useState<BookingData[]>([]);
  const [bookingsFetching, setBookingsFetching] = useState(true);
  ...
  useEffect(() => {
    ...
    // Fetch musician data
    if (musicianId) {
      setMusicianFetching(true);
      api.musician.findOne(musicianId)
        .then(data => {
          setMusicianData(data);
          setMusicianFetching(false);
        })
        .catch(error => {
          console.error("Error fetching musician:", error);
          setMusicianError(error);
          setMusicianFetching(false);
        });
